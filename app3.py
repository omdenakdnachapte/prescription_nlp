...
# Load model and artifacts from the script's directory
@st.cache_resource
...
# Main app
def main():
    # Set save_folder to the directory of this script
    ...
    # Input for dataset path only
    st.sidebar.subheader("Data Configuration")
    data_path = st.sidebar.text_input(
        "Dataset Path",
        value="processed_diseases-priority.csv",
        placeholder="Enter path to dataset CSV (e.g., ./processed_diseases-priority.csv)",
        help="Provide the path to the processed CSV file with disease data"
    )

    # Load data and model
    model, tfidf, label_encoder, faiss_index, df_filtered, common_symptoms = None, None, None, None, None, None
    if os.path.isfile(data_path):
        try:
            ...
            st.success(f"Model and artifacts loaded successfully from {save_folder}! Input dim: {len(tfidf.vocabulary_)}")
        except Exception as e:
            st.error(f"Error loading model or data: {e}")
            return
    else:
        st.error("Invalid dataset path or file not found. Please provide a valid CSV file path.")
        return
    ...
    # Disease lookup section
    st.subheader("Lookup Symptoms by Disease")
    st.markdown("Select a disease to view its associated symptoms from the dataset.")
    ...
    google_form_link = "https://forms.gle/SQYR4DiNB9txyLvm7"  # Replace with your Google Form URL
